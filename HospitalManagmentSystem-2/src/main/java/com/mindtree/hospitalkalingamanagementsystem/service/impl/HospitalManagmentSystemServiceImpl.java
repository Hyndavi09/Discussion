...
//	private ModelMapper modelMapper = new ModelMapper();
	...
	public Patient assignDoctorToPatient(String doctorName, String patientName) throws ServiceException {
		...
		if (!doctorRepository.existsByDoctorName(doctorName)) {
			...
			//salary = (int) (salary + patient.getPatientBillAmount());
	...
	public List<DoctorDto> displayDoctorsBasedOnSalary() {
		...
		List<Doctor> doctorList =doctor.stream().sorted((doctor1,doctor2)-> doctor1.getSalary()-doctor2.getSalary()).collect(Collectors.toList());
	...
	List<Doctor> doctorList=doctors.stream().filter(doctor->doctor.getPatient().size()>3).sorted((doctor1,doctor2)->doctor2.getYearOfExperience()-doctor1.getYearOfExperience())
